...
//Version: 0.1.1-b
 ...
 function parseText(txt) {
    ...
    if (txt) {
        text = txt.simpleText || txt.runs.map(a => a.text).join('');
    }
 ...
 function getPlaylistInfo(json) {
    ...
    if (Array.isArray(results)) {
        for(let i = 0; i < results.length; i++) {
            ...
            let info = result.compactVideoRenderer;
            if (info) {
                expandedInfo.items.push(
                    URL.resolve(BASE_URL, info.navigationEndpoint.commandMetadata.webCommandMetadata.url)
                );
            }
        }
    }
