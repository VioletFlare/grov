...
//https://www.npmjs.com/package/yt-mix-playlist?activeTab=explore
//Version: 0.1.1-b
 ...
 async function main(videoId, options) {
    let mixPlaylist = await getMixPlaylist(videoId, options);

    if (!mixPlaylist) {
        await timeout(50);
        mixPlaylist = await main(videoId, options);
    }
 ...
 async function getMixPlaylist(videoId, options, context, rt = 3) {
     ...
     if (json.contents.twoColumnWatchNextResults.playlist) {
        info = parser.getExpandedPlaylistInfo(json);
     } else {
        info = parser.getPlaylistInfo(json);
     }
